...
describe EtsPdf::Etl::GroupBuilder do
  describe ".call" do
    ...
    let(:group_2) { academic_degree_term_course.groups.find { |group| group.number == 2 } }
    let(:group_3) { academic_degree_term_course.groups.find { |group| group.number == 3 } }
    let(:parsed_lines) do
      [
    ...
    it "calls the period builder with the first collection of group and periods" do
      ...
      expect(EtsPdf::Etl::PeriodBuilder).to have_received(:call).with(group_1, parsed_lines[0..2])
    end

    it "calls the period builder with the second collection of group and periods" do
      described_class.call(academic_degree_term_course, parsed_lines)

      expect(EtsPdf::Etl::PeriodBuilder).to have_received(:call).with(group_2, parsed_lines[3..4])
    end

    it "calls the period builder with the last collection of group and periods" do
      described_class.call(academic_degree_term_course, parsed_lines)

      expect(EtsPdf::Etl::PeriodBuilder).to have_received(:call).with(group_3, parsed_lines[5..6])
    end
  end
end
